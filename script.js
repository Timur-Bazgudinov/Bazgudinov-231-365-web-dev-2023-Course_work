'use strict'
const apiKey = "dc267a28-6cd8-4bda-8e75-3d95286c0e85"
...
function calculateCost(pricePerHour) {
    ...
    let signTranlate = 0
    if (document.querySelector('.souv').checked) {
        signTranlate = 500 * qualPerson
        }
    else {
        document.querySelector('.souv').checked = false;
        }
...
function modal(record, pricePerHour) {
    ...
    const tSouv = document.querySelector('.souv');
    tSouv.addEventListener('change', event => calculateCost(pricePerHour));
